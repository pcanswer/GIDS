...
can_num = 64
feature_num = 3
with open('../data/DoS_dataset.csv','r') as csvfile:
...
np.save("../data/not_encode/DoS_image_data.npy",image)
np.save('../data/not_encode/DoS_image_label.npy',label)

# image = np.load("D:\candata\\new\gear_image_data.npy")
# label = np.load('D:\candata\\new\gear_image_label.npy')
# features = torch.tensor(image, dtype=torch.float32)
# labels = torch.tensor(label, dtype=torch.float32)  #MSELoss -> torch.float32
#
